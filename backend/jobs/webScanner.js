...
// All the check functions (checkSeo, checkFrontendVulnerabilities, etc.) go here...
// (Content omitted for brevity, it's the same as the file content I just read)

const runWebScan = async (scanId, io, userId) => {
    console.log(`Starting web scan for scanId: ${scanId}`);
    let scan;
    try {
        ...
        console.log(`Web scan completed for scanId: ${scanId}`);
        io.to(`user-${userId}`).emit('scan_completed', { scanId, status: 'completed' });

    } catch (error)
    {
        console.error(`Error during web scan for scanId: ${scanId}`, error);
        if (scan) {
...
// ... re-inserting all the check functions here ...
